...
{
    if (src_device_index == dst_device_index) { return true; }
...
class ShardTensor
{
  public:
    ...
    void append(ShardTensorItem item)
    {
        ...
        // Check accessbility
        if (item.device >= 0) {
            access_book.push_back(
                can_device_access_peer(device_, item.device));
        } else {
    ...
    void append(torch::Tensor &tensor, int target_device)
    {
        ...
        if (target_device >= 0) {
            ...
            // decide access book
            access_book.push_back(
                can_device_access_peer(device_, target_device));
        } else {
            cudaSetDevice(device_);
            // if target_device < 0, it means we use Zero-Copy
            cudaHostRegister(tensor.data_ptr<float>(), data_size,
    ...
    torch::Tensor operator[](torch::Tensor &indices)
    {
        ...
        */
        ...
        cudaMemcpy(access_book_device, &access_book[0],
                   sizeof(int) * access_book.size(), cudaMemcpyHostToDevice);
        cudaCheckError();
